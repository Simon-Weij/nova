<script lang="ts">
	...
	async function deleteModel(modelName: string) {
		...
		const response = await apiRequest(`/api/models/${encodeURIComponent(modelName)}`, {
				method: 'DELETE'
			}, 'Delete failed');
...
<main class="flex min-h-screen gap-3 bg-[#ADD8E6] p-3">
	<div class="flex flex-col flex-1 gap-3">
		<div class="flex flex-col justify-center p-4 bg-white rounded-lg">
			<h1 class="text-2xl font-bold text-center border-b">Model Configuration</h1>
			...
			{:else}
				<ul class="flex flex-col gap-2 mt-2">
					{#each models as model (model.name)}
						<li class="flex items-center justify-between gap-2 p-2 text-sm border rounded">
							<div>
								<p>{model.name}</p>
							...
							<button
								...
								class="bg-red-300 rounded text-black cursor-pointer py-2 px-4"
							>
			...
			<button
				...
				class="px-4 py-2 mt-3 text-black bg-blue-300 rounded cursor-pointer hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-60"
			>
				{#if isInstalling}
					Installing…
				{:else}
			...
			<a href="https://ollama.com/library" target="_blank" class="block mt-3 text-xs text-blue-500 hover:underline">
				Browse all models on ollama.com/library ↗
			</a>
		</div>

		<div class="flex items-center justify-center p-4 bg-white rounded-lg"><p>Hello world</p></div>
	</div>
